<template>
    <div class=" w-full max-w-4/5 mx-auto my-20 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 border-b-2 border-white text-white">
        <div class="h-50 flex flex-col justify-center items-center">
            <p class=" text-7xl font-bold">15+</p>
            <p class=" text-md font-normal capitalize">investors</p>
        </div>
        <div class="h-50 flex flex-col justify-center items-center">
            <p class=" text-7xl font-bold">888</p>
            <p class=" text-md font-normal capitalize">skilled speakers</p>
        </div>
        <div class="h-50 flex flex-col justify-center items-center">
            <p class=" text-7xl font-bold">15</p>
            <p class=" text-md font-normal capitalize">coffe breaks</p>
        </div>
        <div class="h-50 flex flex-col justify-center items-center">
            <p class=" text-7xl font-bold">603</p>
            <p class=" text-md font-normal capitalize">our sponsors</p>
        </div>
    </div>
</template>
